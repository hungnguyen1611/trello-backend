...
const createNew = async (userId, data) => {
  try {
    ...
    const getNewBoard = await BoardModal.findOneById(newBoard.insertedId.toString());
    return getNewBoard;
  } catch (error) {
    throw error;
  }
...
const getDetail = async (userId, boardId) => {
  ...
  try {
    ...
    resBoard.columns.forEach(column => column.cards = resBoard.cards.filter(card => card.columnId.equals(column._id)));
...
const getBoards = async (userId, page, itemsPerPage, queryFilters) => {
  try {
    ...
    const result = await BoardModal.getBoards(userId, parseInt(page, 10), parseInt(itemsPerPage, 10), queryFilters);
    return result;
  } catch (error) {
    throw error;
  }
