...
const {
  BrevoProvider
} = require("@/provider/BrevoProvider");
const {
  JwtProvider
} = require("@/provider/JwtProvider");
const {
  env
...
const createUser = async reqBody => {
  try {
    ...
    const newUser = {
      ...
      // Tham số thứ 2 là độ phức tạp băm càng cao băm càng lâu
    ...
    const customSubject = "Trello MERN Stack Advanced: Please verifi your email before using our services!";
...
const login = async reqBody => {
  try {
    ...
    if (!bcrypt.compareSync(reqBody.password, existUser.password)) throw new ApiError(StatusCodes.NOT_ACCEPTABLE, "Your Email or Password is incorrect");
  ...
  try {
    ...
    if (reqBody.current_password && reqBody.new_password) {
      ...
      if (!bcrypt.compareSync(reqBody.current_password, existUser.password)) {
        throw ApiError(StatusCodes.NOT_ACCEPTABLE, "Current password is not incorrect !");
      }
