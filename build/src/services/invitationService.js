...
const updateBoardInvitation = async (userId, invitationId, status) => {
  try {
    ...
    const boardOwnerAndMemberIds = [...getBoard.ownerIds, ...getBoard.memberIds].toString(); // Hoặc dùng concat() để nối mảng
    if (status === BOARD_INVITATION_STATUS.ACCEPTED && boardOwnerAndMemberIds.includes(userId)) {
      throw new ApiError(StatusCodes.NOT_FOUND, "You are already a member of this board!");
    }
    ...
    const updateData = {
      boardInvitation: {
        ...
        // Chuyển sang string trước khi gửi qua model xử lí
        status: status // Accepted hoặc Rejected do FE gửi lên
      }
    };
    //  B1: Cập nhật lại bản ghi trong invitation
    const updatedInvitation = await invitationModel.update(invitationId, updateData);
    console.log("🚀 ~ updateBoardInvitation ~ updatedInvitation.boardInvitation.status:", updatedInvitation.boardInvitation.status);
