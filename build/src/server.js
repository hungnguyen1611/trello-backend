...
const START_SERVER = () => {
  ...
  if (env.BUILD_MODE === "production") {
    ...
    server.listen(PORT, () => {
      console.log(`✅ BackEnd is running at Port ${PORT} — Hello ${process.env.AUTHOR || "Developer"}!`);
    });
  } else {
    server.listen(env.LOCAL_DEV_APP_PORT, env.LOCAL_DEV_APP_HOST, () => console.log(`3 Local: Hi ${process.env.AUTHOR} BackEnd is running successfully http://${env.LOCAL_DEV_APP_HOST}:${env.LOCAL_DEV_APP_PORT}`));
  }
  exitHook(() => {
    CLOSE_DB();
